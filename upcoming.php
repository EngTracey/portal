        ...
        <div class="panel-body">
            <div class="table-responsive">
                <table id="table-basic" class="table" class="table table-striped">
					...
					$query=mysqli_query($conn, "select * from assignments where level_id = '$levelid' and due_date < $todays_date ORDER BY date_given DESC ")or die(mysqli_error($conn));
									while($row=mysqli_fetch_array($query)){
									$id=$row['assign_id'];
									$l_id=$row['level_id'];
									$s_id=$row['subject_id'];
							...
							$query2=mysqli_query($conn, "select * from level where level_id = '$l_id'  ")or die(mysqli_error($conn));
								...
								while($rows3=mysqli_fetch_array($query3)){
									...
									<td ><?php echo $row['date_given']; ?></td> 
										...
										<td ><a rel="tooltip" href='teacher/assignments/<?php echo $row['files']; ?>' class="btn btn-info"> Download</a>                      
									</td>
									</tr>
									<?php }}} ?>
                            </tbody>
